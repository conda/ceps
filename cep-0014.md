...
<tr><td> Updated </td><td> Jun 30, 2026</td></tr>
<tr><td> Discussion </td><td> https://github.com/conda-incubator/ceps/pull/56, https://github.com/conda/ceps/pull/176 </td></tr>
...
build:
  ...
  # a list of regex-based text replacements applied to files in the package
  # during the packaging phase (was the `prefix` replacement done in conda-build)
  post_process:
    # globs selecting which files to process (supports include/exclude mappings
    # and if/then/else conditionals, like other glob fields)
    - files: [glob]
      # regular expression to match within each selected file
      regex: string
      # replacement string; supports capture-group references (`$1`, `${name}`)
      replacement: string
...
- Post-processing MUST run during packaging, _after_ binary relocation and Python
  post-processing, but _before_ the linking checks.
- Matched files MUST be read as UTF-8 text. A file selected by `files` that is not
  valid UTF-8 MUST cause the build to fail.
- Replacement MUST use "replace all" semantics: every non-overlapping match in a
  file is replaced.
- Entries MUST be applied sequentially; later entries observe the results of
  earlier ones.
- In multi-output recipes, an output that does not define `post_process`
  MUST inherit the top-level list. An output that defines its own `post_process`
  MUST fully override the top-level list (MUST NOT be merged).
...
# hash of the file
sha256: hex string
# legacy sha1 sum of the file (test both, prefer sha256)
sha1: hex string
# legacy md5 sum of the file (test both, prefer sha256)
md5: hex string
# relative path from recipe file
patches: [path]
# glob filter applied to the contents of the extracted archive (see "Source filters")
filter: [glob] | { include: [glob], exclude: [glob] }
```
...
patches: [path]
# glob filter; when `path` points to an archive it is applied to the extracted
# contents, otherwise to the copied directory tree (see "Source filters")
filter: [glob] | { include: [glob], exclude: [glob] }
```
...
# absolute or relative path from recipe file
patches: [path]
# glob filter applied to the checked-out tree (see "Source filters")
filter: [glob] | { include: [glob], exclude: [glob] }
# verify that the checked-out commit matches this SHA hash;
# the build MUST fail on mismatch
expected_commit: hex string
```
...
The `filter` field selects a subset of a source's files to copy into the work
directory. It is available on `url`, `git`, and `path` sources. For `url`
sources and `path` sources that point to an archive, the filter is applied to
the contents of the extracted archive; for `git` sources it is applied to the
checked-out tree; for `path` sources that point to a directory it is applied to
the copied tree.

`filter` accepts either a list of include globs, or a mapping with `include`
and/or `exclude` keys:

```yaml
# list form: shorthand for { include: [...] }
filter:
  - cmake/*
  - clang/*
...
The resulting set of files MUST be computed as follows: starting from all files
in the source, keep only those matching at least one `include` glob (if
`include` is omitted or empty, all files are kept), then remove every file
matching at least one `exclude` glob. The list form is equivalent to specifying
only `include`.

### Path interpolation in `patches`

Entries in a source's `patches` list MAY reference build-time directory
variables that are resolved before the patch is applied: `${{ SRC_DIR }}`,
`${{ RECIPE_DIR }}`, and `${{ BUILD_DIR }}`. This allows patches to be located
relative to a directory other than the recipe directory.
...
about:
  ...
  # the license files. Relative paths are found in the source directory _or_ recipe
  # directory, and globs are allowed. Entries MAY reference build-time directory
...
- June 30, 2026: Documented the `filter` field on `url`, `git`, and `path` sources and added the "Source filters" section describing its include/exclude semantics.
- June 30, 2026: Documented additional source fields (`sha1` on URL sources; `submodules` and `expected_commit` on git sources), build-variable interpolation in `patches` and `license_file` paths, and the `build.post_process` regex replacement step.
