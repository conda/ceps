...
### Re: "It would increase the size of the already heavy repodata.json files."
...
### Re: "(Typed) repodata parsers would need to be updated to handle the new field."
...
If neither sharded repodata nor a `run_exports.json` file is present build tools can assume no run export patches exist.
