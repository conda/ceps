...
<tr><td> Discussion </td><td> https://github.com/conda/ceps/pull/111, https://github.com/conda/ceps/pull/165, https://github.com/conda/ceps/pull/177, https://github.com/conda/ceps/pull/181 </td></tr>
...
This new field must be selectable by `MatchSpec` syntax using the `extras` keyword inside square brackets, the value of which MUST be a string or a list of strings targeting group names. Each of these strings:

- MAY be wrapped with whitespace, which MUST be ignored.
- MUST be a string literal whose value matches the same `[a-z0-9_.+-]{1,64}` grammar as the `extra_depends` keys. A `MatchSpec` whose `extras` value contains a name that does not match this grammar is invalid and MUST be rejected.
- MUST be compared case-sensitively and MUST NOT be normalized.

The syntax for lists MUST follow the same syntax as [YAML 1.2 flow sequences style](https://yaml.org/spec/1.2.2/#flow-sequences), without newlines. In other words, a comma-separated sequence of strings wrapped in square brackets. All nodes MUST be strings.

If an optional dependency group is matched by the `extras` field in `MatchSpec`, its list of dependencies MUST be unioned with the regular `depends` field and resolved accordingly. Referencing a group name that is absent from a candidate record's `extra_depends` MUST NOT be an error and MUST NOT contribute additional dependencies.
...
- 2026-07-15: Changed `extra-depends` to `extra_depends` for consistency with other underscore-separated field names in `index.json`.
- 2026-07-06: Clarified the `MatchSpec` `extras` selector syntax (single vs. list form, optional quoting, whitespace), and specified that selector names must obey the group-name grammar (case-sensitive, no normalization), that non-conforming names are rejected, and that referencing an absent group is a no-op.
