...
- 2026-07-02: Clarified the `MatchSpec` `extras` selector syntax (single vs. list form, optional quoting, whitespace), and specified that selector names must obey the group-name grammar (case-sensitive, no normalization), that non-conforming names are rejected, and that referencing an absent group is a no-op.
