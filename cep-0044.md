...
<tr><td> Updated </td><td> Jul 2, 2026</td></tr>
...
The `extras` value is either a single group name or a bracketed, comma-separated list of group names (for example, `pkg[extras="group-name"]` or `pkg[extras=[group-a, group-b]]`). Individual names in the list MAY be quoted with single or double quotes (`pkg[extras=["group-a", 'group-b']]`); the quotes are optional and carry no semantic difference. Whitespace surrounding list items is ignored. Each name MUST match the same `[a-z0-9_.+-]{1,64}` grammar as the `extra-depends` keys; names are compared case-sensitively and MUST NOT be normalized. A `MatchSpec` whose `extras` value contains a name that does not match this grammar is invalid and MUST be rejected. Referencing a group name that is absent from a candidate record's `extra-depends` is not an error; it simply contributes no additional dependencies.
...
- Jul 2, 2026: Clarified the `MatchSpec` `extras` selector syntax (single vs. list form, optional quoting, whitespace), and specified that selector names must obey the group-name grammar (case-sensitive, no normalization), that non-conforming names are rejected, and that referencing an absent group is a no-op.
