...
A channel name is defined as the

Channel names MUST only consist of lowercase ASCII letters, numbers, hyphens, periods and underscores. They MUST start and end with a letter or a digit. They MUST NOT contain two consecutive separators (hyphen, underscore, period). In other words, they MUST match this regex:  `^[a-z0-9]+((-|_|.)[a-z0-9]+)*$`.

The maximum length of a channel name MUST NOT exceed 128 characters.
...
Platform or subdir names MUST only consist of lowercase ASCII letters, numbers, hyphens, underscores and periods. They MUST start and end with a letter or a number. They MUST NOT contain two consecutive separators (hyphen, underscore, period). They MUST match this regex: `^[a-z0-9]+((-|_|.)[a-z0-9]+)*$`.
