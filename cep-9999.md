...
Since `conda-index` already processes the package metadata to generate `repodata.json`, it would be trivial to also generate `run_exports.json` and serve it next to it.
...
> See the validation schema in [`run_exports.schema.json`](#TBD).
