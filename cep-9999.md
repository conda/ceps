...
<tr><td> Updated </td><td> Feb 24, 2023</td></tr>
...
need to be standardized if they are going to be kept in use. 
...
supports new settings and/or CLI flags. Namely:

- `base_prefix`: override the assumed `base` environment location. This is nowadays available as
  `root_prefix` but overriding this with environment variables is buggy in `conda` and needs to be
  fixed.
